object Exercises {
    ...
    */
    ...
    def sortByHeavyWeight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val weights = ballsArray.map { case (material, (radius, density)) =>
            val volume = (4.0 / 3.0) * java.lang.Math.PI * math.pow(radius, 3)
            val weight = density * volume
            (weight, material)
        }.toSeq

        weights.sortBy(_._1).map(_._2)
    }
}